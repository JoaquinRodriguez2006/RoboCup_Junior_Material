...
while robot.step(timeStep) != -1:
    if not (distancia_sensor1.getValue() < 0.1):
        ...
        if not (distancia_sensor2.getValue() < 0.1):
            rotar_enclavado(90)
            print("Deteccion de obstaculo en el LADO IZQUIERDO")
        else:
            print("Deteccion de obstaculo en el LADO DERECHO")
            rotar_enclavado(-90)
